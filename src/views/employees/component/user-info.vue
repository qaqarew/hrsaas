<template>
  <div class="user-info">
    <i class="el-icon-coffee-cup" @click="$router.push('/employees/print/' +userId +'?type=personal')" />
    <!-- 个人信息 -->
    <el-form label-width="220px">
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item v-loading="loading" label="员工头像">
            <!-- 放置上传图片 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item v-loading="loading" label="员工照片">
          <!-- 放置上传图片 -->
          <uploadImg :default-url="employeesPic" @on-success="uploadAvatarSuccess" />
...
export default {
  ...
  methods: {
    async UserDeatilById() {
      try {
        this.loading = true
        const res = await getUserDeatilById(this.userId)
        if (res.staffPhoto) {
    ...
    async saveUserInfo() {
      try {
        await saveUserDeatilById(this.userInfo)
